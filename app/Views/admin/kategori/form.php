...
<div class="max-w-container_max_width mx-auto">
    ...
    <div class="bg-surface-container-lowest border border-surface-variant rounded-lg p-[24px] level-1-surface max-w-3xl">
        <form action="<?= isset($kategori) ? base_url('admin/' . tenant()->pkm_slug . '/kategori/update/' . $kategori['kategori_id']) : base_url('admin/' . tenant()->pkm_slug . '/kategori/store') ?>" method="POST" class="space-y-6">
            <?= csrf_field() ?>
