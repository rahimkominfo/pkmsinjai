    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-center self-start md:self-auto">
            <?php if (isset($list_pkm)): ?>
                <form action="<?= base_url('admin/' . tenant()->pkm_slug . '/kategori') ?>" method="GET" class="flex items-center gap-2">
            ...
            <?php endif; ?>
            <a href="<?= base_url('admin/' . tenant()->pkm_slug . '/kategori/create') ?>" class="bg-primary text-on-primary px-4 py-2 rounded font-data-table text-data-table hover:shadow-sm hover:-translate-y-[1px] transition-all flex items-center gap-2">
        ...
        <table class="w-full text-left border-collapse min-w-[600px]">
            <thead>
                <tr class="border-b border-surface-variant text-outline-variant font-label-sm text-label-sm uppercase tracking-wider">
                    ...
                    <th class="p-3">Induk Kategori</th>
            ...
            <tbody>
                <?php if (!empty($kategori)): ?>
                    ...
                    <tr class="border-b border-surface-container-low hover:bg-[#eff4ff] transition-colors">
                        <td class="p-3">
                            <div class="font-data-table text-data-table text-on-surface"><?= esc($row['nama']) ?></div>
                        </td>
                        <td class="p-3 font-body-md text-body-md text-on-surface-variant"><?= esc($row['slug']) ?></td>
                        <td class="p-3 font-body-md text-body-md text-on-surface-variant">
                            <?= $row['nama_induk'] ? esc($row['nama_induk']) : '<span class="text-outline-variant italic">Utama</span>' ?>
                        </td>
                        <?php if (tenant()->pkm_id === 'super'): ?>
                        <td class="p-3 font-body-md text-body-md text-on-surface-variant">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="<?= tenant()->pkm_id === 'super' ? '6' : '5' ?>" class="p-4 text-center font-body-md text-outline-variant">Belum ada data kategori.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
